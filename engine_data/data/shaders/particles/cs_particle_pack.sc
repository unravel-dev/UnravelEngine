/*
 * Pack particle sim SoA into 96-byte instance rows for vs_particle_instanced.
 * One thread per draw slot; s_order[i] selects the source particle.
 */

#include <bgfx_compute.sh>

BUFFER_RO(s_sim, vec4, 0);
BUFFER_RO(s_order, uint, 1);
BUFFER_WO(s_instances, vec4, 2);
BUFFER_RO(s_color_lut, vec4, 3);
BUFFER_RO(s_color_speed_lut, vec4, 4);
BUFFER_RO(s_ease_lut, vec4, 5);

uniform vec4 u_pack0; // opacity, color_intensity, avg_scale, render_mode
uniform vec4 u_pack1; // pivot.xy, particle_count, features
uniform vec4 u_pack2; // scale3d.xyz, tex_sheet_cycles
uniform vec4 u_pack3; // tex_tiles.xy, tex_sheet_randomize, pad
uniform vec4 u_pack4; // size_by_speed min/max, inv_size_span, size_speed_min
uniform vec4 u_pack5; // inv_color_span, color_speed_min, pad, pad
...
#define u_particle_count    uint(u_pack1.z)
...
void main()
{
    uint draw_i = gl_GlobalInvocationID.x;
    if(draw_i >= u_particle_count)
    {
        return;
    }

    uint particle_i = s_order[draw_i];
    uint base = particle_i * 5u;
    ...
    bool need_speed = ((u_features & (FEAT_COLOR_SPD | FEAT_SIZE_SPD)) != 0u);
    float particle_speed = 0.0;
    if(need_speed)
    {
        vec3 initial_velocity = end0 - start;
        vec3 final_velocity = end1 - end0;
        vec3 current_velocity = mix(initial_velocity, final_velocity, tt);
        particle_speed = length(current_velocity / lifespan);
    }
    ...
    if((u_features & FEAT_TEXSHEET) != 0u)
    {
        ...
        uint total_frames = uint(u_tex_tiles.x) * uint(u_tex_tiles.y);
        total_frames = max(total_frames, 1u);
    ...
    uint out_base = draw_i * 6u;
    s_instances[out_base + 0u] = vec4(world_pos, u_pivot_x);
    s_instances[out_base + 1u] = rotation;
    s_instances[out_base + 2u] = vec4(u_scale3d * scale, u_pivot_y);
    s_instances[out_base + 3u] = vec4(uv_offset, uv_scale);
    s_instances[out_base + 4u] = color;
    s_instances[out_base + 5u] = vec4(u_render_mode, 0.0, 0.0, 0.0);
}
