 ...
 * (the adaptive program packs four 16-lane probes into each 64-lane group). The traced
 * COUNT itself rides to the kernel in the list head's y lane - a partial final adaptive
 * group bounds-checks its tail slots against it; a value (not a bit pattern) because
 * typed-UAV float traffic may canonicalise NaN payloads, which a bit-cast sentinel would
 * be one driver away from becoming. The head's x lane is the FIRST coordinate and is
 * preserved by the staging store.
 *
