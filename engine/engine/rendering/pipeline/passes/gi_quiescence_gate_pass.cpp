...
void gi_quiescence_gate_pass::service_stats_snapshot(const gfx::texture::ptr& vis_memo, uint32_t attr_resolution)
{
    ...
    bgfx::setImage(1, snapshot_texture_->native_handle(), 0, bgfx::Access::Write, bgfx::TextureFormat::R32U);
...
auto gi_quiescence_gate_pass::ensure_buffers() -> bool
{
    ...
    if(!bgfx::isValid(indirect_))
    {
        indirect_ = bgfx::createIndirectBuffer(entry_count);
    }
    if(!bgfx::isValid(ring_))
    {
        ...
        ring_ = bgfx::createDynamicIndexBuffer(ring_header_slots + ring_sample_slots,
...
auto gi_quiescence_gate_pass::run(gfx::render_view& rview, const run_params& params) -> bool
{
    ...
    gfx::set_image_3d(0, vis_memo->native_handle(), 0, bgfx::Access::ReadWrite, bgfx::TextureFormat::R32U);
    bgfx::setBuffer(1, ring_, bgfx::Access::ReadWrite);
    bgfx::setBuffer(2, indirect_, bgfx::Access::Write);
    // The per-level surface counts size the light-voxel launch (the shader's tight relight note).
    bgfx::setBuffer(5, clipmap_gpu.get_surface_list_buffer(), bgfx::Access::Read);
    ...
    if(probes_bound)
    {
        bgfx::setBuffer(6, clipmap_gpu.get_world_probe_select(), bgfx::Access::Read);
    }
    ...
    bgfx::dispatch(pass.id, program_.program->native_handle(), 1, 1, 1);
