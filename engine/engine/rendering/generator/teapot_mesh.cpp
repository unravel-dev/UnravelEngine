...
teapot_mesh_t::triangles_t::triangles_t(const teapot_mesh_t& mesh) noexcept
    : mesh_t{&mesh}
    , index_{0}
    , patch_mesh_{std::make_shared<bezier_mesh_t<4, 4>>(ANONYMOUS::teapotData[0],
...
void teapot_mesh_t::triangles_t::next()
{
    ...
    if(triangles_.done())
    {
        ...
        if(!done())
        {
            patch_mesh_ = std::make_shared<bezier_mesh_t<4, 4>>(ANONYMOUS::teapotData[index_],
...
teapot_mesh_t::teapot_mesh_t(int segments) noexcept
    : segments_{segments}
    , patch_vertex_count_{count(bezier_mesh_t<4, 4>{ANONYMOUS::teapotData[0], {segments, segments}}.vertices())}
{
}
