...
public:
    ...
    gen_iterator_t() noexcept : generator_{nullptr}, value_{}
    {
    }

    /// Iterator to the given generator.
    gen_iterator_t(generator_t& generator) noexcept : generator_{&generator}, value_{}
    {
        if(generator_->done())
            generator_ = nullptr;
        else
    ...
    /// @throws std::out_of_range If the iterator is out of range.
    const typename gen_iterator_t::value_type& operator*() const
    {
        if(!generator_)
            throw std::out_of_range("Iterator out of range!");
        return value_;
...
{
    return gen_iterator_t<generator_t>{generator};
}
...
{
    return gen_iterator_t<generator_t>{};
}
} // namespace generator
