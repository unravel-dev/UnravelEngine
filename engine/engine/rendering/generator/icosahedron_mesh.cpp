...
icosahedron_mesh_t::triangles_t::triangles_t(const icosahedron_mesh_t& mesh)
    : mesh_{&mesh}
    , i_{0}
    , triangle_mesh_{std::make_shared<triangle_mesh_t>(ANONYMOUS::vertices[ANONYMOUS::triangles[0][0]],
                                                       ANONYMOUS::vertices[ANONYMOUS::triangles[0][1]],
                                                       ANONYMOUS::vertices[ANONYMOUS::triangles[0][2]],
                                                       mesh.segments_)}
...
{
    return i_ == static_cast<int>(ANONYMOUS::triangles.size());
}
...
void icosahedron_mesh_t::triangles_t::next()
{
    ...
    if(triangles_.done())
    {
        ...
        if(!done())
        {
            triangle_mesh_ = std::make_shared<triangle_mesh_t>(ANONYMOUS::vertices[ANONYMOUS::triangles[i_][0]],
                                                               ANONYMOUS::vertices[ANONYMOUS::triangles[i_][1]],
                                                               ANONYMOUS::vertices[ANONYMOUS::triangles[i_][2]],
                                                               mesh_->segments_);
