...
static bool ImOsIsDebuggerPresent()
{
    ...
    FILE* fp = fopen("/proc/self/status",
                     "rb"); // Can not use ImFileLoadToMemory because size detection of /proc/self/status would fail.
    if(fp == NULL)
        return false;
...
auto ImGui_ImplOSPP_MapCursor(ImGuiMouseCursor cursor) -> os::cursor::type
{
    static const std::map<ImGuiMouseCursor, os::cursor::type> cursor_map = {
        ...
        {ImGuiMouseCursor_Progress, os::cursor::type::arrow_wait},
...
static void ImGui_ImplOSPP_UpdateMonitors()
{
    ...
    for(int n = 0; n < display_count; n++)
    {
        ...
        monitor.DpiScale = os::display::get_content_scale(n);
...
static void ImGui_ImplOSPP_CreateWindow(ImGuiViewport* viewport)
{
    ...
    win_flags |= os::window::resizable;
    win_flags |= (viewport->Flags & ImGuiViewportFlags_NoDecoration) ? os::window::borderless : 0;
    ...
    vd->Window = IM_NEW(unravel::render_window)(std::move(os_win));
    vd->WindowOwned = true;
...
static void ImGui_ImplOSPP_ShowWindow(ImGuiViewport* viewport)
{
    ...
    os::set_hint("HINT_WINDOW_ACTIVATE_WHEN_SHOWN",
    ...
    if(!vd->WindowShownAtLeastOnce)
    {
        vd->WindowShownAtLeastOnce = true;
        vd->Window->get_window().set_opacity(vd->WindowOpacity);
    }
}
