...
inline ImTextureID ToId(const gfx::texture& _handle, uint8_t _mip = 0, uint8_t _flags = IMGUI_FLAGS_ALPHA_BLEND)
{
    ...
    if(_handle.info.cubeMap)
    {
        _flags |= IMGUI_FLAGS_CUBEMAP;
    }
...
inline ImTextureID ToId(const asset_handle<gfx::texture>& _handle,
                        uint8_t _mip = 0,
                        uint8_t _flags = IMGUI_FLAGS_ALPHA_BLEND)
{
...
{
    return ImVec2{float(tex.info.width * (tex.info.cubeMap ? 4 : 1)), float(tex.info.height * (tex.info.cubeMap ? 3 : 1))};
}
