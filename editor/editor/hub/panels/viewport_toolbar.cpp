...
auto get_text_color_on_accent() -> ImU32
{
    const ImVec4 accent = get_accent_color();
    const float luminance = 0.299f * accent.x + 0.587f * accent.y + 0.114f * accent.z;
...
void draw_item(const ImRect& bb, const item_desc& desc, bool is_hovered, bool is_held)
{
    ...
    if(desc.is_active)
    {
        draw_list->AddRectFilled(bb.Min, bb.Max, ImGui::GetColorU32(get_accent_color()), rounding);
    }
    if(is_hovered || is_held)
    {
        draw_list->AddRectFilled(bb.Min, bb.Max, is_held ? ITEM_HELD_COLOR : ITEM_HOVERED_COLOR, rounding);
    }
    const float text_alpha = (is_hovered || desc.is_active) ? 1.0f : ITEM_TEXT_ALPHA;
    ImU32 text_color = ImGui::GetColorU32(ImGuiCol_Text, text_alpha);
    if(desc.text_color != 0)
    {
        text_color = desc.text_color;
    }
    if(desc.is_active)
    {
        text_color = get_text_color_on_accent();
    }
...
auto toggle(const char* id, const char* text, bool is_active, const char* tooltip, const char* width_text) -> bool
{
    item_desc desc{};
    desc.text = text;
    desc.width_text = width_text;
