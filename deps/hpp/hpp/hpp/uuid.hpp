...
template <typename TChar>
[[nodiscard]] constexpr basic_string_view<TChar> to_string_view(TChar const* str) noexcept
{
	if(str)
		return str;
	return {};
...
[[nodiscard]] constexpr basic_string_view<typename StringType::value_type, typename StringType::traits_type>
...
enum class uuid_variant
{
	...
	// N bit pattern: 0xxx
	// > the first 6 octets of the UUID are a 48-bit timestamp (the number of 4 microsecond units of time
	// since 1 Jan 1980 UTC); > the next 2 octets are reserved; > the next octet is the "address family"; >
...
{
	none = 0,				 // only possible for nil or invalid hpp
...
inline void swap(hpp::uuid& lhs, hpp::uuid& rhs) noexcept
{
	lhs.swap(rhs);
}

// --------------------------------------------------------------------------------------------------------------------------
// namespace IDs that could be used for generating name-based hpp
// --------------------------------------------------------------------------------------------------------------------------
...
private:
	...
	void process_characters(basic_string_view<CharT, Traits> const str)
	{
		for(uint32_t c : str)
		{
...
// this implementation is unreliable for good hpp
class uuid_time_generator
{
	using mac_address = std::array<unsigned char, 6>;

	optional<mac_address> device_address;
...
public:
	[[nodiscard]] uuid operator()()
	{
		if(get_mac_address())
		{
			std::array<hpp::uuid::value_type, 16> data;

			auto tm = get_time_intervals();

			auto clock_seq = get_clock_sequence();

			auto ptm = reinterpret_cast<hpp::uuid::value_type*>(&tm);
			...
			return hpp::uuid{std::cbegin(data), std::cend(data)};
		}
...
{
template <>
struct hash<hpp::uuid>
{
	using argument_type = hpp::uuid;
	...
	[[nodiscard]] result_type operator()(argument_type const& uuid) const
	{
		...
		return static_cast<result_type>(hasher(hpp::to_string(uuid)));
#else
